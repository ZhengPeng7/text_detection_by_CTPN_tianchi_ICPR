    ...
    for image_path_idx in range(len(image_paths))[:100]:
        ...
        with open(lab_ax_path, 'r') as fin:
            for line in fin.readlines():
                ...
                # Get nearest point pairs
                idx_dis_min = np.argmin(
                    np.sum((point_axis[0][1:] - point_axis_1)**2, axis=1)
                ) + 1
                point_axis_nearest = point_axis[0][idx_dis_min]
                point_axis_for_correction = sorted([point_axis_1,
                                                    point_axis_nearest],
                                                   key=lambda x: x[1])
                print(point_axis_1, point_axis_nearest)
                correcting_angle = np.arctan((point_axis_for_correction[1][1] -
                                             point_axis_for_correction[0][1]) /
                                             (point_axis_for_correction[1][0] -
                                              point_axis_for_correction[0][0]))
                correcting_angle = np.rad2deg(correcting_angle) - 90
